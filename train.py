...
if __name__ == '__main__':
    ...
    resume_training_timestamp = d['resume_training_timestamp'] # CHANGE HERE
    ...
    if ablation:
        ...
        max_epoch = d["ablation-conf"]["max_epoch"]
        batch_size = d["ablation-conf"]["batch_size"]
        image_size = d["ablation-conf"]["image_size"]
        volume_depth = d["ablation-conf"]["volume_depth"]
        ablation_study_size = d["ablation-conf"]["ablation_study_size"]
    ...
    monitor = d['monitor']
    initial_threshold = d['initial_threshold']
    mode=d['mode']
    freq=d['freq']
    early_stop = d['early_stop']

    precision = d['precision'] # 16 bits or 32 bits

    learning_rate = d['learning_rate']
    lr_schedular = d['lr_schedular']
    enable_class_weight = d['enable_class_weight']

    model_ext = d['model_ext']
    cache_file_name_fmt = f"{cache_root}/oasis_cache_{batch_size:03}{volume_depth:03}{image_size:03}{n_ch:03}"

    if not is_running_in_colab():
        distributed = False

    task_type = d['task_type'] # "binary"
    ...
    model_name=d['model_name'] # CHANGE HERE !!!!!!!
    ...
    my_proc = Process(target=log_system_usage, args=(artifact_root,))
    ...
    def analyse_result(df, prefix):
        try:
            ...
            sns.heatmap(cm, annot=True, cmap='Blues', xticklabels=class_ids, 
