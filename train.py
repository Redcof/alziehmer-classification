...
if __name__ == "__main__":
    ...
    experiment_id_path = os.path.join(log_root, "current_experiment_timestamp_id.txt")

    if resume_training and resume_training_timestamp is None:
        assert os.path.exists(experiment_id_path), "Unable to resume experiment. No previous timestamp detected."
    ...
    # my_proc = Process(target=loop_log_system_usage, args=(artifact_root,))
    ...
    class OASISTorchDataset(Dataset):
        ...
        def item_generator(self):
            ...
            for class_dir in self.classnames:
                ...
                for slices, label in zip(
                    grouped_files.values(), [class_dir] * len(grouped_files.values())
                ):
        ...
        def load_volume(self, image_files: list, label=None, idx=None):
            ...
            volume = (
                ...
                .astype(self.dtype)
                .reshape((OASISTorchDataset.VOLUME_DEPTH, *self.image_size, self.nch))
            )
            if label is None:
                return volume
            else:
    ...
    def prepare_model():
        ...
        log("Compile model")
        model.compile(
            optimizer=optimizer, loss=loss_function, metrics=metrics, run_eagerly=True
        )
    ...
    with file_writer.as_default():
        tf.summary.text(
            f"Corner Case", f"Actual={actual_class} Predicted={predicted_class}", step=0
        )
